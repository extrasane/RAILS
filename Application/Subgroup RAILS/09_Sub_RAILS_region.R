#### 09_Sub_RAILS_region.R — RAILS by Census region
#### Run inside the AoU Researcher Workbench.
#### Subgroup variable: region. Runs all four regions (Northeast, Midwest,
#### South, West) via the wrapper fun.sub.rails.threeway and row-binds them.
####
...
########################################################################
## RAILS by region
## region is the subgroup variable; sex stays IN the model. The wrapper loops
## all four regions, building each region's PUMS margins + nsiz internally, and
## row-binds the cell-level results tagged with subgroup_run.
########################################################################

names_univar <- c("agegroup", "sex", "edu", "homeown", "income", "race_eth")

result_region <- fun.sub.rails.threeway(
...
## Diagnostics: each region's calibrated model and its weight total
result_region %>%
  group_by(subgroup_run) %>%
  summarise(n_cells    = n(),
            total_rails = sum(d_rails * weight, na.rm = TRUE),
            calibrated  = first(calibrated_terms),
            .groups = "drop")

########################################################################
## Attach weights to individual-level AoU data
## Join on the model covariates PLUS region (the subgroup key).
## All d_* columns are already per-individual.
########################################################################

dt_out_region <- dt_aou %>%
  left_join(
    result_region %>%
      rename(
      ...
      select(all_of(names_univar), region, w_unweighted, w_cal1, w_cal2,
             w_nps1, w_nps2, w_nps1_rake, w_nps2_rake, w_subrails,
             selected_terms, calibrated_terms),
    by = c(names_univar, "region")
  )
...
write_excel_csv(dt_out_region, "../data/dt_sub_aou_region.csv")

# system(paste0("gsutil cp ../data/dt_sub_aou_region.csv ", my_bucket, "/data/"), intern = TRUE)
