...
nss  = 5.5e3
naou = 3.5e4
alpha <- c(1,-0.1,0.2,-0.5,-1,-1.5,
           -0.2,0.7,
           0,0,0,0)
beta <- c(-2.2,-0.1,-1.5,-1.8,-2.5,-3.5,
          0,0)
gamma <- c(-6.3,0.12,-0.4,0.3,0.6,0.9,
           -0.2,0.4,
           0.5,-0.3,-0.6,-0.9)
...
if("pre_pop_10.30.csv" %in% list.files(getwd(), recursive = TRUE)){
  pre_pop <- read.csv("pre_pop_10.30.csv")
}else{
  pre_pop <- subpre.function(nsiz=100000000,alpha,beta,gamma,names_var)
  write.csv(pre_pop,"C:/Work/2024 Fall/Weighting/pre_pop_10.30.csv")
}

if("pre_all_10.30.csv" %in% list.files(getwd(), recursive = TRUE)){
  pre_all <- read.csv("pre_all_10.30.csv")
}else{
  pre_all <- subpre.function(nsiz=10000000,alpha,beta,gamma,c("catx1","catx2","catx3","catx4","catx5"))
  write.csv(pre_all,"C:/Work/2024 Fall/Weighting/pre_all_10.30.csv")
}
```
    ...
    tryCatch(
      {
        ...
        n_stepwise <- c(unselect = n_max - n_uni,
...
### Simulations
if("10_30.RData" %in% list.files(getwd(), recursive = TRUE)){
  sim_result <- readRDS("10_30.RData")
}else{
  sim_result <- replicate(n,suppressWarnings(sim.fun(nsiz,alpha,beta,gamma,
                                                     names_var,pre_pop,pre_all,true_y)))
  save.image("C:/Work/2024 Summer/AoU_Weighting/Simulation/10_30.RData")
}
```
