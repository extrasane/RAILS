...
if("pre_pop_10.31.csv" %in% list.files(getwd(), recursive = TRUE)){
  pre_pop <- read.csv("pre_pop_10.31.csv")
}else{
  pre_pop <- subpre.function(nsiz=100000000,alpha,beta,gamma,names_var)
  write.csv(pre_pop,"C:/Work/2024 Fall/Weighting/pre_pop_10.31.csv")
}

if("pre_all_10.31.csv" %in% list.files(getwd(), recursive = TRUE)){
  pre_all <- read.csv("pre_all_10.31.csv")
}else{
  pre_all <- subpre.function(nsiz=10000000,alpha,beta,gamma,c("catx1","catx2","catx3","catx4","catx5"))
  write.csv(pre_all,"C:/Work/2024 Fall/Weighting/pre_all_10.31.csv")
}
```
...
### Simulations
if("10_29.RData" %in% list.files(getwd(), recursive = TRUE)){
  sim_result <- readRDS("10_29.RData")
}else{
  sim_result <- replicate(n,suppressWarnings(sim.fun(nsiz,alpha,beta,gamma,
                                                     names_var,pre_pop,pre_all,true_y)))
  save.image("C:/Work/2024 Summer/AoU_Weighting/Simulation/10_29.RData")
}
```
